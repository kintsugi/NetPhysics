...
public:
	...
	*/
	...
	ComponentTypeHandle* getComponentTypeHandle(const ComponentType type);
	...
	*/
	template<class T>
	std::vector<T*> getComponents(HandleManager *handleManager, const ComponentType type);
	...
	*/
	bool removeComponents(HandleManager* handleManager, const ComponentType type);
...
private:
	Handle handle;
	std::unordered_map<ComponentType, ComponentTypeHandle> components;
...
template<class T>
std::vector<T*> GameObject::getComponents(HandleManager *handleManager, const ComponentType type) {
	std::vector<T*> ret;
	auto got = components.find((ComponentType)handle.type);
	if (got != components.end()) {
