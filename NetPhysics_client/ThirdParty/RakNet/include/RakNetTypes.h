...
struct RAK_DLL_EXPORT AddressOrGUID
{
	...
	SystemIndex GetSystemIndex(void) const { if (rakNetGuid != RakNetGUID()) return rakNetGuid.systemIndex; else return systemAddress.systemIndex; }
	bool IsUndefined(void) const {return rakNetGuid==RakNetGUID() && systemAddress==SystemAddress();}
	void SetUndefined(void) {rakNetGuid=RakNetGUID(); systemAddress=SystemAddress();}
	...
	inline bool operator==( const AddressOrGUID& right ) const {return (rakNetGuid!=RakNetGUID() && rakNetGuid==right.rakNetGuid) || (systemAddress!=SystemAddress() && systemAddress==right.systemAddress);}
};
